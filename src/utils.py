    ...
    try:
        ...
        for nd in name_dicts:
            if 'preferred' in nd and nd['preferred']:
                name_dict = nd
        has_middle_name = ('middle' in name_dict) and (name_dict['middle'] is not None) and len(name_dict['middle']) > 0
        author_df['name'] = name_dict['first'] + (' '+name_dict['middle']+' ' if has_middle_name else ' ') +  name_dict['last']
